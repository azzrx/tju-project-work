...
### 4. gcn.py：图卷积神经网络节点分类 (GCN)

**基于图卷积神经网络（Graph Convolutional Network, GCN）**的节点分类实验，使用经典的 Cora 引文网络数据集，解决图结构数据上的半监督学习问题。

    **实验背景**

*   **功能**

    *   使用 PyTorch Geometric 提供的 Planetoid 接口加载 Cora 数据集。

    *   构建两层 GCN 模型，对节点邻域信息进行特征聚合。

    *   在仅使用少量训练节点的情况下，对全图节点进行类别预测。

    *   输出训练集、验证集和测试集上的分类准确率，用于评估模型性能。
